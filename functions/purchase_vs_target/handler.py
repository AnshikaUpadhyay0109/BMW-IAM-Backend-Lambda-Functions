...
def _run_athena_query(sql: str) -> str:
    ...
    print(f"[purchase_vs_target] Query submitted → {execution_id}")
    ...
    for attempt in range(MAX_POLL_ATTEMPTS):
        ...
        if state in ("SUCCEEDED", "FAILED", "CANCELLED"):
            break
        print(f"[purchase_vs_target] Attempt {attempt + 1}: state={state}, waiting…")
...
def lambda_handler(event, context):
    log_event("purchase_vs_target", event)
    try:
        execution_id   = _run_athena_query(SQL_QUERY)
        output_s3_path = f"{S3_OUTPUT_LOCATION}{execution_id}.csv"
        print(f"[purchase_vs_target] SUCCESS → {output_s3_path}")
        return success_response({
            "query_name":         "purchase_vs_target",
            "query_execution_id": execution_id,
            "output_s3_path":     output_s3_path,
        })
    except Exception as exc:
        return error_response(f"purchase_vs_target failed: {exc}")
